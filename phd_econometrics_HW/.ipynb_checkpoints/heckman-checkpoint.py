    ...
    **kwargs:
    ...
    def __init__(self, endog, exog, exog_select, **kwargs):
        ...
        if np.asarray(exog_select).ndim==2:
            exog_select_1dnan_placeholder =                 [np.nan if any(np.isnan(row)) else 1 for row in np.asarray(exog_select)]
        else:  # assume ==1
        ...
        self.endog = self.data.endog =             np.asarray(
    ...
    def fit(self, method='twostep', start_params_mle=None, method_mle=None, maxiter_mle=None, **kwargs_mle):
        ...
        **kwargs_mle:
        ...
        if method=='twostep':
            if start_params_mle is not None or method_mle is not None or maxiter_mle is not None or                 len(kwargs_mle.keys())>0:
                warnings.warn('The user chose to estimate the Heckman model by Two-Step,' +                     ' but MLE arguments were provided. Extraneous MLE arguments will be ignored.')
    ...
    def _fit_twostep(self):
        ...
        sigma2Hat = step2res.resid.dot(step2res.resid) / self.nobs_uncensored +             (betaHat_inverse_mills**2 * sum(delta)) / self.nobs_uncensored
    ...
    def loglikeobs(self, params_all):
        ...
        ll_obs_observed[~D] = 0
        ll_obs_notobserved =             np.multiply(1-D, 
